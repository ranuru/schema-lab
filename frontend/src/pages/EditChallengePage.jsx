...
export default function EditChallengePage() {
  ...
  if (loading) return <div style={{ padding: 40, color: '#a0aec0' }}>Loading…</div>

  return (
    <div style={{ maxWidth: 760, margin: '0 auto', padding: '40px 16px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 8 }}>
        <h1 style={{ fontSize: 28, fontWeight: 700, color: '#f7fafc' }}>Edit Challenge</h1>
        <Link to={`/challenges/${id}`} style={{ fontSize: 13, color: '#718096' }}>Cancel</Link>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div style={fieldStyle}>
          ...
          <input style={inputStyle} value={form.title} onChange={setField('title')} />
        </div>
        ...
        <div style={fieldStyle}>
          ...
          <textarea style={monoStyle} value={form.starterCode} onChange={setField('starterCode')} />
        </div>
        <div style={fieldStyle}>
          <label style={labelStyle}>Harness Code</label>
          <textarea style={monoStyle} value={form.harnessCode} onChange={setField('harnessCode')} />
        </div>
